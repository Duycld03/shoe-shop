...
public class OrderDAO {
			...
			if (conn != null && !conn.isClosed()) {
				...
				while (rs.next()) {
					Order order = new Order(rs.getString("OrderID"), rs.getFloat("TotalAmount"),
							...
							rs.getString("CustomerID"), rs.getString(7), rs.getString("StaffID"));
					list.add(order);
				}
			} else {
	...
	public float getTotalRevenue() {
		...
		String sql = "select SUM(TotalAmount) as TotalRevenue from Orders where PaymentStatus = 'Paid' and OrderStatus = 'Success'";
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public float getTotalRevenueByMonth(int month) {
		float totalRevenue = 0;
	...
	public int geTotalOrderByMonth(int month) {
		int totalOrder = 0;
		int currentYear = Calendar.getInstance().get(Calendar.YEAR);
		String sql = "SELECT Count(OrderDate) AS totalOrder\n"
				+ "                     FROM Orders \n"
				+ "                     WHERE YEAR(OrderDate) = ? \n"
				+ "					 and MONTH(OrderDate) = ?\n"
				+ "                     GROUP BY MONTH(OrderDate)";
	...
	public static void main(String[] args) {
		...
		List<Order> list = d.getOrderbyStaffID("ST1");
